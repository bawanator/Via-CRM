// Vía OS MCP tool registry — shared by BOTH transports:
//   - mcp/server.ts             stdio (Claude Code / Claude Desktop local)
//   - src/app/api/mcp/[token]   Streamable HTTP (claude.ai custom connector,
//                               which syncs to the desktop and mobile apps)
//
...
// change to this channel. The actor (created_by/updated_by) is resolved from
// MCP_ACTOR_EMAIL because service-role requests have no auth.uid().
//
// Protocol note: on stdio, stdout belongs to the transport — diagnostics here
// use console.error only, which is also the right stream on the server.
...
export async function resolveActorId(db: Db): Promise<string | null> {
...
export function buildServer(db: Db, actorId: string | null): McpServer {
