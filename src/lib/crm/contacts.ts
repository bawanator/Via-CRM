...
export type ContactFilter = {
  ...
  search?: string; // ilike over full_name / company / email
};

async function attachStats(db: Db, contacts: ContactRow[]): Promise<ContactWithStats[]> {
...
export async function listContacts(db: Db, filter: ContactFilter = {}): Promise<ContactWithStats[]> {
  let query = db.from("contacts").select("*").order("full_name");
...
export async function resolveContactId(db: Db, idOrName: string): Promise<string> {
  ...
  throw new Error(
    `Ambiguous contact "${idOrName}" — candidates: ${matches.map((m) => `${m.full_name}${m.company ? ` (${m.company})` : ""} [${m.id}]`).join(", ")}`,
  );
}
